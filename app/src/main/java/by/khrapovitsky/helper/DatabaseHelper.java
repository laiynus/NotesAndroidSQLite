...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Note readNote(int id){
        ...
        Cursor cursor = db.query(TABLE_NOTES, new String[] { KEY_ID,
                        KEY_NOTE_TEXT, KEY_LAST_DATE_MODIFY }, KEY_ID + "=?",
    ...
    public List<Note> getAllNotes() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                note.setLastDateModify(cursor.getString(2));
                noteList.add(note);
    ...
    public Cursor getAllForAdapter(){
        SQLiteDatabase db = this.getWritableDatabase();
        return db.rawQuery("SELECT _id,noteText,lastDateModify FROM " + TABLE_NOTES,null);
    }
